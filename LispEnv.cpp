...
namespace Lisp {
  ...
  LispEnv::LispEnv() : parent_(LispEnvRef()) {
	...
	fEnv_["SET!"]  = LispObjRef(static_cast<LispObj*>(new LispObj(PrimType(LispEnv::set_fn))));
  }
  ...
  LispObjRef LispEnv::set_fn(LispObjRef cons, LispEnvRef env) {
	// cdr of cons should be a symbol
	if (is_symbol(car(cons)))
	  env->set(((LispSymbol)(boost::get<SymbolType>(*car(cons)))).first, cdr(cons));
	return cdr(cons);
  }
}
